...
class IGVaultParser(BaseParser):
    """
    IGVault (igv.com) — P2P marketplace, ~$0.25–0.35/M.
    """
    platform = "IGVault"

    def __init__(self, url: str = None):
        self.url = url or config.IGVAULT_URL
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                cards = await page.query_selector_all(
                    "[class*='offer'], [class*='listing'], [class*='product'], article, tr"
                )
                ...
                for card in cards:
                    try:
                        ...
                        url  = href if (href and href.startswith("http")) else f"https://www.igv.com{href or ''}"
                        ...
                        log.debug(f"IGVault card: {e}")

            except PWTimeout:
                log.warning("IGVault timeout")
            except Exception as e:
                log.error(f"IGVault: {e}")
            finally:
                await browser.close()

        log.info(f"IGVault: {len(listings)} EU silver listings")
        return listings
