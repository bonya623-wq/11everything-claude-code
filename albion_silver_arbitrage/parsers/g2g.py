...
_CARDS   = ["[class*='offer-item']", "[data-testid='offer-card']", "article", "[class*='product-list'] li"]
_TITLES  = ["[class*='title']", "[class*='name']", "h3", "h4"]
_PRICES  = ["[class*='listed-price']", "[class*='price']", "[class*='cost']"]
_SELLERS = ["[class*='seller']", "[class*='brand']", "[class*='user']"]
            ...
            try:
                ...
                for card in cards:
                    try:
                        ...
                        log.debug(f"G2G card: {e}")

            except PWTimeout:
                log.warning(f"G2G timeout")
            except Exception as e:
                log.error(f"G2G: {e}")
            finally:
                await browser.close()

        log.info(f"G2G: {len(listings)} EU silver listings")
        return listings
