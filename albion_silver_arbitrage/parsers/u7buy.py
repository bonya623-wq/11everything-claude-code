...
class U7BUYParser(BaseParser):
    """
    U7BUY — storefront aggregator, ~$0.22–0.28/M.
    Usually shows fixed-price packages per server.
    """
    platform = "U7BUY"

    def __init__(self, url: str = None):
        self.url = url or config.U7BUY_URL
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                cards = await page.query_selector_all(
                    "[class*='product-item'], [class*='offer-item'], [class*='goods-item'], "
                    "[class*='silver-item'], [class*='package'], article, li.item"
                )
                ...
                for card in cards:
                    try:
                        ...
                        price_el = await card.query_selector("[class*='price'], strong, b, [class*='amount']")
                        if not price_el:
                            continue
                        ...
                        url  = href if (href and href.startswith("http")) else f"https://www.u7buy.com{href or ''}"
                        ...
                        log.debug(f"U7BUY card: {e}")

            except PWTimeout:
                log.warning("U7BUY timeout")
            except Exception as e:
                log.error(f"U7BUY: {e}")
            finally:
                await browser.close()

        log.info(f"U7BUY: {len(listings)} EU silver listings")
        return listings
